...
const renderHistorial = async () => {
    ...
    logs.forEach(log => {
        ...
        let detailFormat = `De '${log.old_value || 'N/A'}' a '${log.new_value || 'N/A'}'`;
        
        const tr = document.createElement('tr');
        tr.innerHTML = `
            <td style="color: var(--text-secondary);">${formatDate(log.timestamp)}</td>
            <td><span style="color: var(--accent); font-weight: 500; text-transform: capitalize;">${actionFormat}</span></td>
            <td>${detailFormat}</td>
            <td style="font-size:0.75rem; color:var(--text-muted);">${log.user_id.split('-')[0]}...</td>
        `;
        tbody.appendChild(tr);
    });
};
...
const renderEvidencias = async () => {
    ...
    evidences.forEach(ev => {
        // Formatear el hash para que no rompa el diseño si es muy largo
        const shortHash = ev.hash ? ev.hash.substring(0, 16) + '...' : 'N/A';
        ...
        card.innerHTML = `
            <div style="display: flex; justify-content: space-between; margin-bottom: 1rem;">
                ...
                <span class="badge badge-desindexado" style="font-size: 0.7rem;">${ev.status || 'Registrado'}</span>
            </div>
            <div style="font-size: 0.875rem; color: var(--text-secondary); margin-bottom: 1.5rem; line-height: 1.8;">
                ...
                <p><strong>Hash SHA-256:</strong> <span style="font-family: monospace; color: var(--accent);">${shortHash}</span></p>
            </div>
            <div style="display: flex; gap: 1rem;">
                <button class="btn btn-secondary" style="flex: 1; font-size: 0.8rem;" onclick="alert('Funcionalidad en desarrollo. ID Evidencia: ${ev.id}')">Ver Registro Completo</button>
                <button class="btn btn-secondary" style="flex: 1; font-size: 0.8rem;" onclick="alert('Funcionalidad en desarrollo. ID Evidencia: ${ev.id}')">Descargar</button>
            </div>
        `;
        grid.appendChild(card);
    });
